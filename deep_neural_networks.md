...
has its own set of supported hyper-parameters:
  ...
  - Hessian:
      ...
      - **max\_iterations**: Maximum number of iterations (default:
        2000)
...
accepts the following configuration parameters, among others:
  ...
  - **padding**: An integer or list of 2 integers specifying the amount
    ...
    with either <span class="title-ref">same</span> or
    <span class="title-ref">valid</span> (Tensorflow notation)
  ...
  - **trainable**: If set to <span class="title-ref">false</span>, the
    ...
    all trainable layers (not only
    <span class="title-ref">Conv2d</span>), and might be useful when
...
implementations:
  ...
  - **nll**: Log-Likelihood (NLL) loss function. Useful to train a
    classification problem with \(C\) classes. Accepts an optional list
    of weights to be applied to each class.
...
\((h,w,d\)), where \(h \le H\), \(w \le W\) and \(d \le D\), or slice it
into 2D tiles along the \(xy\)-plane with shape \((h,w)\), or even
...
The following options can be set:
  ...
  - **padding**: Symmetric padding to be added along each dimension
    ...
    with <span class="title-ref">padded\_sizes</span>.
  - **padded\_sizes**: Total slice size after padding (defaults to
    <span class="title-ref">sizes</span>). Useful in case the desired
    padding is asymmetric. Cannot be specified together with
    <span class="title-ref">padding</span>.
  ...
  - **discard\_remainders**: Whether to discard regions of the input
    ...
    <span class="title-ref">false</span>, i.e., leftovers are padded up
    to <span class="title-ref">sizes</span> and added to the sample
    list).
...
![slicer\_padding.png](figures/dnn/slicer_padding.png)
...
![slicer\_flow.png](figures/dnn/slicer_flow.png)
...
The following transformations are supported:
  ...
  - **resize**: Resizes the sample to a given size using bilinear
    ...
    > Usage: `resize: [Sx, Sy]`, where \(S_x \times S_y\) is the desired
    > sample size.

  - **center\_crop**: Crops the sample at the center to a given output
    size.
    
    > Usage: `center_crop: [Sx, Sy]`, where \(S_x \times S_y\) is the
    > output size.

  - **jitter\_crop**: Crops the sample in each direction \(i\) by
    \(c \times S_i / 2\), where \(c\) is a random variable uniformly
    sampled from \(c \in [0, C_\textrm{max})\).
  ...
  - **random\_zoom**: Randomly zooms in by \(c \times S_i / 2\) in each
    direction \(i\), where \(c\) is a random variable uniformly sampled
  ...
  - **rotate**: Rotates the sample clockwise by a given fixed angle.
    ...
    > Usage: `rotate: phi`, where \(\phi\) is the rotation angle.
...
This transform can be configured with the
<span class="title-ref">number</span> of random patches and their linear
<span class="title-ref">size</span>, as for example:
...
![random\_patches.png](figures/dnn/random_patches.png)

### Checkpoints section

In order to save model checkpoints out to disk during training, one must
add the <span class="title-ref">checkpoints</span> object to the
<span class="title-ref">json</span> config file. This section can also
be used to load the model from file before running training. Accepted
model file formats are `.pt` (from libtorch) and `.h5` (HDF5 from
Keras/TF).
...
```
  ...
  - **interval**: When set to \(N\), will save model checkpoints at
    every \(N\) epochs (defaults to 1).
...
requires the loss function to be like the
<span class="title-ref">NLL</span>, which expects a target with such
shape.
...
of the same shape (e.g., <span class="title-ref">MSE</span>,
<span class="title-ref">BCE</span>, etc) must be used.
...
2)  the 2D stochastic Poisson equation with boundary conditions
    \(u(0,y)=1\), \(u(1,y)=0\) and \(\partial u / \partial \hat n = 0\)
    on other boundaries
3)  the 3D stochastic Poisson equation with boundary conditions
    \(u(0,y,z)=1\), \(u(1,y,z)=0\) and
    \(\partial u / \partial \hat n = 0\) on other boundaries

**Burgers' PDE solver**

The Burgers' equation in one spatial dimension can be written
as

\[\frac{\partial u}{\partial t} + u \frac{\partial u}{\partial x} = \nu \frac{\partial^2 u}{\partial x^2},\]

where \(\nu\) is the diffusion coefficient. When \(\nu = 0\), the
Burgers' equation is said to be <span class="title-ref">inviscid</span>.
...
where \(d\) is the domain size in the
<span class="title-ref">x</span>-dimension, and the frequency \(c\) is
sampled from the interval \([c_{min}, c_{max}]\) using one of the
supported distributions (see `bc_type` parameter below).
...
<span class="title-ref">function</span> as `Burgers_PDE`, and the
...
where \(\nu(x,y)\) is the <span class="title-ref">diffusivity</span>
function. The forcing function \(f\) is assumed to be zero. The loss
<span class="title-ref">function</span> must be set to `Poisson2D_PDE`
or `Poisson3D_PDE`:
...
![unet2d\_multigrid.png](figures/dnn/unet2d_multigrid.png)
