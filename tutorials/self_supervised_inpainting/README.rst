...
To demonstrate how the current self-supervised method of inpainting can be used with fewer labels, 
we will experiment with subsets of the original training dataset, and assess how the smaller datasets affect our classifier accuracy and by how much. After a new directory containing, for example, 80% of the original dataset is created, one can point to it on the config file by doing:
